---
description: Read or set the bridle execution mode (sync, async, or autopilot)
argument-hint: "[sync|async|autopilot]"
---

Read or set the bridle execution mode by editing `.claude/rules/bridle-mode.md`.

With no argument, print the current mode. With `sync`, `async`, or `autopilot`, update the rule and confirm the change.
...
1. **Resolve the rule.** Look for `.claude/rules/bridle-mode.md`.
   ...
   - If the rule is missing but `.claude/rules/` exists, scaffold it from `${CLAUDE_PLUGIN_ROOT}/templates/.claude/rules/bridle-mode.md` and report that the rule was created at the sync default.

2. **Parse `$ARGUMENTS`:**
   - Empty → read the rule, find the `**Active mode: <X>**` line, print the current mode.
   - `sync`, `async`, or `autopilot` → swap the `**Active mode: <X>**` line to the chosen value. Use exact-string replacement; do not rewrite the rest of the rule.
   - Anything else → print valid options (`sync`, `async`, `autopilot`) and stop.

3. **Confirm the change.** Print:
   - Before / after of the changed line.
   - One-sentence reminder of what changes:
     - **sync** — agent asks for feedback at every phase boundary.
     - **async** — agent iterates without asking; stops and reports if anything is unclear.
...
- Sync is the default; the rule ships with sync set.
